...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        if boxRPB != "none":
            ...
            # Avoid precomputing this cache on a hardcoded device. We populate it
            # lazily in `_get_rpb_matrix()` using the device of the current input.
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        ...
        if _is_dynamo_compiling() or self.compilable_stored_size == (
            H,
            W,
        ):
